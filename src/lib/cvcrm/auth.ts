...
import type {
  SolicitarCodigoVerificacaoResponse,
  GerarTokenResponse,
} from '@/types';

export class CVCRMAuthManager {
  private config = loadCVCRMConfig();
  ...
  async getToken(): Promise<string> {
    try {
      // Verificar cache
      const cachedToken = await this.cache.get<string>(this.tokenKey);
    ...
    try {
      ...
      if (!response.ok) {
        throw new ExternalAPIError(
          `Erro ao solicitar código: ${response.status}`,
          response.status
        );
      }

      const data: SolicitarCodigoVerificacaoResponse = await response.json();
      ...
      logger.info(
        { validadeMinutos: data.validadeMinutos },
  ...
  private async generateToken(code: string): Promise<string> {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new ExternalAPIError(
          `Erro ao gerar token: ${response.status}`,
          response.status
        );
      }

      const data: GerarTokenResponse = await response.json();
   ...
   */
  async storeVerificationCode(code: string): Promise<void> {
    const codeKey = REDIS_KEYS.verificationCode(this.config.usuario);
 ...
 */
let authManager: CVCRMAuthManager | null = null;

export function getAuthManager(): CVCRMAuthManager {
  if (!authManager) {
