...
export const consultarChamadosTool = {
...
// ========== HANDLERS ==========
export async function handleCriarChamado(args) {
...
export async function handleConsultarChamados(args) {
    const client = getCVCRMClient();
    const { unidadeId, situacao } = consultarChamadosSchema.parse(args);
    const filtros = {};
    if (unidadeId)
        filtros.unidadeId = unidadeId;
    if (situacao)
...
//# sourceMappingURL=assistencia.js.map