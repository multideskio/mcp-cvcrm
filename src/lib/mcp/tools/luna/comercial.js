...
// ========== TOOL DEFINITION ==========
export const listarEmpreendimentosTool = {
...
// ========== HANDLER ==========
export async function handleListarEmpreendimentos(args) {
    ...
    if (filtros.cidade) {
        empreendimentosFiltrados = empreendimentosFiltrados.filter((emp) => emp.endereco?.cidade?.toLowerCase().includes(filtros.cidade.toLowerCase()));
    }
    // Buscar unidades disponíveis para cada empreendimento
    const empreendimentosComDetalhes = await Promise.all(empreendimentosFiltrados.map(async (emp) => {
        // Buscar unidades disponíveis
        // Nota: Endpoint pode ser GET /empreendimentos/{id}/unidades?disponivel=true
        try {
            const unidades = await client.request(`/empreendimentos/${emp.id}/unidades?disponivel=true`);
            ...
            if (filtros.valorMin && valorMaximo < filtros.valorMin)
                return null;
            if (filtros.valorMax && valorMinimo > filtros.valorMax)
                return null;
            return {
                id: emp.id,
                nome: emp.nome,
