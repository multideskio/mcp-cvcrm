...
// ========== TOOL DEFINITIONS ==========
export const consultarParcelasTool = {
...
// ========== HANDLERS ==========
export async function handleConsultarParcelas(args) {
    ...
    const parcelas = await client.request(`/reservas/${reserva.id}/parcelas`);
    ...
    if (situacao && situacao !== 'todas') {
        parcelasFiltradas = parcelas.filter((p) => {
            if (situacao === 'em_aberto')
                return p.situacao === 'em_aberto';
            if (situacao === 'vencidas')
                return p.situacao === 'vencida';
            if (situacao === 'pagas')
                return p.situacao === 'paga';
            return true;
        });
    }
    // Calcular resumo
...
export async function handleGerarBoleto(args) {
    const client = getCVCRMClient();
    const { parcelaId, enviarEmail } = gerarBoletoSchema.parse(args);
    const boleto = await client.request('/financeiro/boletos/segunda-via', {
        method: 'POST',
        body: JSON.stringify({
            parcelaId,
