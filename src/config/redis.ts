...
export const REDIS_KEYS = {
  ...
  token: (dominio: string, usuario: string) => 
    `cvcrm:token:${dominio}:${usuario}`,
  
  /** Chave para armazenar código de verificação temporário */
  verificationCode: (usuario: string) => 
    `cvcrm:code:${usuario}`,
  
  /** Chave para cache de empreendimentos */
  empreendimentos: () => 
    `cvcrm:cache:empreendimentos`,
  
  /** Chave para cache de workflows */
  workflows: (funcionalidade: string) => 
    `cvcrm:cache:workflows:${funcionalidade}`,
